...
int main(const int argc, const char *argv[]) {
    auto console_sink = MakeSharedPtr<spdlog::sinks::stdout_color_sink_mt>();
    console_sink->set_level(spdlog::level::trace);
    console_sink->set_pattern("%+");

    auto file_sink = MakeSharedPtr<spdlog::sinks::basic_file_sink_mt>("/tmp/netmiddleware.log", true);
    file_sink->set_level(spdlog::level::trace);
    file_sink->set_pattern("%+");

    auto logger_registry = MakeSharedPtr<Log::LoggerRegistry>(spdlog::sinks_init_list{console_sink, file_sink});
    ...
    auto module_registry = MakeSharedPtr<ModuleRegistry>();
    ...
    auto intf_mngr = MakeSharedPtr<InterfaceManager>(Module::Name::IFACE_MANAGER, module_registry, rpc_net_channel);
    auto lag_mngr = MakeSharedPtr<LagManager>(Module::Name::LAG_MANAGER, module_registry, rpc_net_channel);
    auto port_mngr = MakeSharedPtr<PortManager>(Module::Name::PORT_MANAGER, module_registry, rpc_net_channel);
    auto vlan_mngr = MakeSharedPtr<VlanManager>(Module::Name::VLAN_MANAGER, module_registry, rpc_net_channel);
    ...
    auto root_req_hdl = MakeSharedPtr<RequestHandle>(Module::Name::REQUEST_HANDLE, module_registry,
                                                     XPath::ROOT_NODE_START);
    auto interface_req_hdl = MakeSharedPtr<RequestHandle>(Module::Name::REQUEST_HANDLE, module_registry, "interface");
    root_req_hdl->add(interface_req_hdl);
    auto agg_ethernet_req_hdl = MakeSharedPtr<AggregateEthernetRequestHandle>(
        Module::Name::REQUEST_HANDLE, module_registry, interface_req_hdl);
    interface_req_hdl->add(agg_ethernet_req_hdl);
    auto ethernet_req_hdl = MakeSharedPtr<EthernetRequestHandle>(Module::Name::REQUEST_HANDLE, module_registry,
                                                                 interface_req_hdl);
    interface_req_hdl->add(ethernet_req_hdl);
    auto platform_req_hdl = MakeSharedPtr<RequestHandle>(Module::Name::REQUEST_HANDLE, module_registry, "platform");
    root_req_hdl->add(platform_req_hdl);
    auto platform_port_req_hdl = MakeSharedPtr<PortRequestHandle>(Module::Name::REQUEST_HANDLE, module_registry);
    platform_req_hdl->add(platform_port_req_hdl);
    auto protocol_req_hdl = MakeSharedPtr<RequestHandle>(Module::Name::REQUEST_HANDLE, module_registry, "protocol");
    root_req_hdl->add(protocol_req_hdl);
    auto lacp_req_hdl = MakeSharedPtr<LacpRequestHandle>(Module::Name::REQUEST_HANDLE, module_registry,
                                                         protocol_req_hdl);
    protocol_req_hdl->add(lacp_req_hdl);
    auto rstp_req_hdl = MakeSharedPtr<RstpRequestHandle>(Module::Name::REQUEST_HANDLE, module_registry,
                                                         protocol_req_hdl);
    protocol_req_hdl->add(rstp_req_hdl);
    auto vlan_req_hdl = MakeSharedPtr<VlanRequestHandle>(Module::Name::REQUEST_HANDLE, module_registry, root_req_hdl);
    root_req_hdl->add(vlan_req_hdl);
    ConnectionManagement::Server cm;
    cm.addOnPostConnectionHandler("config_running_update",
                                  [&module_registry, &root_req_hdl](const String &path, String data_request,
                                                                    String &return_data) {
                                      if (path != ConnectionManagement::URIRequestPath::Config::CONFIG_UPDATE) {
                                          spdlog::debug("Unexpected URI requested '{}' - expected '{}'", path,
                                                        ConnectionManagement::URIRequestPath::Config::CONFIG_UPDATE);
